...
using namespace std;
	...
	RubicsCube::RubicsCube(glm::vec3 position,float angle,float hwRelation,float near, float far) : Scene(position,angle,hwRelation,near,far)
	{
		direction = 1;
		factor = 1;
		rotCount = 0;
	...
	void RubicsCube::updateIndices(int type)
	{
		...
		case 3:
			if(type<0)
				...
				cell = ((type) + (squarsNum % 2))*squarsNum*squarsNum;
			if(direction == -(type))
			{
			  diffX = 1;
			  diffY = squarsNum;
		...
		case 4:
			if(type<0)
				...
				cell = ((type) + (squarsNum % 2))*squarsNum;
			if(direction == -(type))
			{
